...
n0 = sF.non0(μx_t0)                         # Non-zero indices of first [μ,ρ]
...
def simulate(itraj,TrajFolder):
    ...
    for ij in range(n0.shape[1]):
        ...
        wt = np.zeros(model.NSteps1, model.NSteps2, model.NSteps3, dtype=complex)          # weight for each response 
        wt[:,0,0] = np.ones(wt.shape[0])*μx_t0[iF0,iB0]                                    # initial weights according to μx_t0 elements

        """Keeping track of feynman diagrams"""
        kside = np.zeros(model.NSteps1, model.NSteps2, model.NSteps3, dtype=int)
        ksign = np.zeros(model.NSteps1, model.NSteps2, model.NSteps3, dtype=int)
